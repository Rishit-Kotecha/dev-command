...
export async function writeState(state, message, opts = {}) {
  const r = repo();
  await ensureBranch(r);
  const cur = await readState();
  const curAt = Number(cur.state.updatedAt || 0);
  if (opts.expect != null && curAt && Number(opts.expect) !== curAt) {
    const e = new Error('The saved data changed since this page loaded. Reload to get the latest.'); e.status = 409; e.detail = { state: cur.state }; throw e;
  }
  const hadData = (cur.state.projects || []).length + (cur.state.agents || []).length > 0;
  const emptyIncoming = !(state.projects || []).length && !(state.agents || []).length;
  if (hadData && emptyIncoming && !opts.force) {
    const e = new Error('Refusing to replace saved projects and team with an empty list.'); e.status = 409; e.detail = { state: cur.state }; throw e;
  }
